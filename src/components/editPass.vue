<template>
    <!--强制修改密码弹出框-->
    <el-dialog :title=" title || $t('pass.title2')" :visible.sync="pwdChangeDlgVis" :area="510" :close-on-click-modal="false" top="middle" :before-close="closeDlg">
  ...
  export default {
    ...
    methods: {
      ...
      passNoInit (rule, value, callback) {
        ...
        let riskPass = this.getRiskPassVali( value , (window.localStorage.user || this.user),valiTypeReset);
        ...
        this.$refs.editForm.validate((valid) => {
          if (valid) {
            http.getRequest(this.url, 'post',{
              username: window.localStorage.user || this.user ,
